...
export function Sidebar({ 
  ...
  return (
    <>
      ...
      <aside 
        className={`h-[100dvh] shrink-0 border-r border-gray-200 bg-white flex flex-col p-4 fixed left-0 top-0 z-50 transition-transform md:transition-all duration-300 ${
          isMobileOpen ? 'translate-x-0 w-64 shadow-2xl' : '-translate-x-full md:translate-x-0'
        } ${isCollapsed ? 'md:w-20 md:items-center' : 'md:w-64'}`}
      >
        <div className={`flex items-center mb-8 mt-2 ${isCollapsed ? 'md:justify-center w-full' : 'w-full px-2 justify-between'}`}>
          {isCollapsed ? (
            <button 
          ...
          {links.map((link) => (
            <NavLink
              ...
              className={({ isActive }) =>
                ...
                } ${isCollapsed ? 'md:justify-center md:p-3 md:w-12 md:mx-auto px-4 py-3 justify-start' : 'justify-start px-4 py-3'}`
              }
            >
              {({ isActive }) => (
