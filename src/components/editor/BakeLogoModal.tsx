...
export function BakeLogoModal({ isOpen, onClose, imageUrl, onSaveImage }: BakeLogoModalProps) {
  ...
  const modalContent = (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[9999] flex flex-col p-0 sm:p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div 
        className="bg-white border-0 sm:border border-slate-200 rounded-none sm:rounded-3xl shadow-2xl w-full max-w-6xl mx-auto flex flex-col h-full sm:max-h-[92vh] overflow-hidden" 
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-serif text-lg sm:text-xl text-slate-900 font-bold tracking-wide">Logo Studio & Realistic Fabric Baking</h3>
                <span className="text-[10px] uppercase font-bold tracking-widest bg-amber-50 text-amber-700 border border-amber-200 px-2 py-0.5 rounded-full">Studio</span>
              </div>
              <p className="text-xs text-slate-500">Position brand graphics and bake them realistically into garment cloth folds, creases & weave</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden bg-slate-50">
          ...
          <div className="flex-1 relative flex flex-col items-center justify-center p-4 min-h-[350px] overflow-hidden bg-slate-50 bg-[radial-gradient(#cbd5e1_1px,transparent_1px)] [background-size:16px_16px]">
            <div 
              ref={containerRef}
            ...
            >
              ...
              {isBaking && (
                <div className="absolute inset-0 bg-white/85 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center z-30 animate-in fade-in">
                  ...
                  <h4 className="text-slate-900 font-bold text-lg mb-1">Baking Logo Realistically...</h4>
          ...
          <div className="w-full md:w-96 shrink-0 border-t md:border-t-0 md:border-l border-slate-200 p-5 flex flex-col justify-between bg-white overflow-y-auto space-y-4">
              ...
              <div>
                ...
                {!logoSrc ? (
                  <button
                    ...
                    className="w-full border-2 border-dashed border-slate-300 hover:border-amber-500 hover:bg-amber-50/40 rounded-2xl p-6 flex flex-col items-center justify-center gap-2 text-center transition-all cursor-pointer group bg-slate-50/60"
                  >
                    <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 group-hover:text-amber-600 group-hover:bg-amber-100 transition-all">
                      <UploadCloud size={20} />
                    </div>
                    <div>
                      <p className="text-xs font-bold text-slate-800">Upload Logo or Graphic</p>
                      <p className="text-[11px] text-slate-400 mt-0.5">PNG, SVG, or JPG (transparent PNG recommended)</p>
                    </div>
                  </button>
                ) : (
              ...
              {logoSrc && !resultImage && (
                <div className="space-y-4 pt-2 border-t border-slate-100 animate-in fade-in">
                  ...
                  <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-2 text-amber-800 text-[11px]">
                    <Sliders size={14} className="shrink-0 text-amber-600" />
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-500 uppercase font-bold tracking-wider text-[10px]">Scale / Size</span>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {rotation !== 0 && (
                          <button 
                            ...
                            className="text-[10px] text-amber-600 hover:underline ml-1 font-semibold"
                          >
                            Reset
                          </button>
                        )}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs text-slate-800 outline-none focus:bg-white focus:border-amber-500 cursor-pointer"
                    >
            ...
            <div className="space-y-2 pt-4 border-t border-slate-200">
              {!resultImage ? (
                <Button
                  ...
                  isLoading={isBaking}
                  className="w-full bg-gradient-to-r from-amber-500 to-rose-500 hover:from-amber-600 hover:to-rose-600 text-white py-3 rounded-full text-xs uppercase tracking-widest font-bold shadow-md shadow-amber-500/20 transition-all flex items-center justify-center gap-2 border-0 disabled:opacity-50"
                >
                  <Sparkles size={15} />
                ...
                <div className="space-y-2">
                  <Button
                    ...
                    disabled={isSaving}
                    isLoading={isSaving}
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-full text-xs uppercase tracking-widest font-bold shadow-md shadow-emerald-600/20 transition-all flex items-center justify-center gap-2 border-0"
                  >
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider transition-colors"
                    >
                      Adjust Placement
                    </button>
                    <button
                      ...
                      onClick={handleBake}
                      disabled={isBaking}
                      className="flex-1 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider transition-colors flex items-center justify-center gap-1.5"
                    >
                      <Sparkles size={13} />
                      Regenerate
                    </button>
